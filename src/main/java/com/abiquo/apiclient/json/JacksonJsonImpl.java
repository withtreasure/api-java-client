...
public class JacksonJsonImpl implements Json
{
    ...
    @Override
    public <T> T read(final Reader reader, final Class<T> clazz) throws IOException
    {
        return mapper.readValue(reader, mapper.constructType(clazz));
    }

    @Override
    public <T> T read(final Reader reader, final TypeToken<T> type) throws IOException
    {
